# start/register Specification

## Purpose

Регистрация пользователя Telegram на `/start` (включая разовый триал), навигация по разделам через инлайн-меню; тематические разделы с учётом gate подписки, «Подписка» — экран тарифов.
