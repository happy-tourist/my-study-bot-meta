...
- Разовый пробный период при первой регистрации на `/start` (3 минуты; в тексте — «3 дня» с пометкой минут).
...
- Gate: «Машины» и «Дома» только при активной подписке; отказ по-русски; «Подписка» без gate.
- Временный минутный режим expiry: окна remind 3/2/1 и cron каждую минуту (до возврата дневного прод-режима с ЮKassa).
- **BREAKING** для локальной проверки: сброс SQLite-базы, чтобы триал и новые поля применились к чистому состоянию.
...
- Триал только при первом создании пользователя; повторный `/start` не выдаёт второй триал.
...
- Runtime: sibling `../my-study-bot` — модель (флаг триала), `/start`, меню/тарифы, gate helper, планировщик expiry.
- Локальная SQLite: сброс файла БД при apply.
