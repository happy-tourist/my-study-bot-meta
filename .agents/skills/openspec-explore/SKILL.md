...
- нет подходящей библиотеки / пакета в bot (и нет канона «берём X» в requirements / skills)
- нет принятого решения по домену (уроки, медиа, платежи подписки, расписание, …)
- нет инфраструктуры / env / провайдера (Telegram Bot Token, payment provider, storage, …) и нет зафиксированного выбора
- конфликт со стеком или skills (нужен подход вне aiogram/SQLAlchemy/SQLite/Telegram и т.п. без решения)
- нет capability path / доменного контракта, и его нельзя вывести из существующих доменов sibling AGENTS
- продуктовый выбор с несколькими несовместимыми вариантами без default в проекте
- legal/security/ops вопрос (PII, секреты, TG_TOKEN/DB_URL на VPS), который должен решить человек
...
- `requirements.txt` / deps bot
- существующие flows (`/start`, User/subscription, handlers, FSM, keyboards, middleware)
